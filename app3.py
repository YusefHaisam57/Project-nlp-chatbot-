    ...
    if st.button("🚀 Run"):
        if not st.session_state.pdf_uploaded:
            ...
            if selected_action == "Generate MCQ Questions":
                st.session_state.chat.append(("user", "Generate MCQ Questions"))
                with st.chat_message("user"):
                    st.markdown("Generate MCQ Questions")
                ...
                st.session_state.chat.append(("user", "Generate True/False Questions"))
                with st.chat_message("user"):
                    st.markdown("Generate True/False Questions")

                with st.chat_message("assistant"):
                    st.markdown("Generating True/False questions...")

                tf_response = generate_true_false(
                    st.session_state.pdf_text[:1500],
                    st.session_state.num_questions
                )

                st.session_state.chat.append(("assistant", tf_response))
                st.session_state.history.append(tf_response)

                with st.chat_message("assistant"):
                    st.markdown(tf_response)
                    st.download_button(
                        label="💾 Download True/False Questions as .txt",
                        data=tf_response.encode("utf-8"),
                        file_name="true_false_questions.txt",
                        mime="text/plain"
                    )
